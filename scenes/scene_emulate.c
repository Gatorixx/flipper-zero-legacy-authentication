...
static void mfdes_nfc_emulate_callback(MfDesDeviceEventType type, void* context) {
    ...
    if(type == MfDesAuthenticationError)
    furi_crash();
...
void desfire_app_scene_emulate_on_enter(void* context) {
    ...
    FuriString* message = furi_string_alloc_set_str("Emulating UID: ");
    furi_string_cat(message, instance->device->uid);
    widget_add_string_multiline_element(widget, 80, 26, AlignCenter, AlignCenter, FontPrimary, furi_string_get_cstr(message));
...
bool desfire_app_scene_emulate_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        ...
        if(event_index == MfDesAuthenticationFinish) {
            ...
            widget_add_string_element(instance->widget, 90, 26, AlignCenter, AlignCenter, FontPrimary, "Success");
            //TODO zatim se to vrati ale chce to pridat nejakou uspech obrazovku
            scene_manager_previous_scene(scene_manager);
        } else if(event_index == MfDesAuthenticationError) {
            widget_add_string_element(instance->widget, 90, 26, AlignCenter, AlignCenter, FontPrimary, "Error");
            mfdes_nfc_stop_emulation(instance);
