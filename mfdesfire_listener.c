...
NfcCommand mfdes_listener_select_application_cmd(MfDesDevice* device, const uint8_t data){
    ...
    //TODO zavolat neco jako device_set_specific_context(device, MfDesListenerStateSelected);
...
NfcCommand mfdes_listener_legacy_authentication_cmd(MfDesDevice* device, const uint8_t data){
    ...
    memcpy(init_vector, device->initial_vector, sizeof(device->initial_vector)); // To prevent overwriting the app vector

    furi_hal_random_fill(rndB, sizeof(rndB));
    ...
    //TODO zavolat neco jako device_set_specific_context(device, MfDesListenerStatePhase1Sent);
...
void mfdes_rotate(uint8_t* dest, const uint8_t* src, MfDesRotation rotation){
    ...
    for(uint8_t i = 0; i < MFDES_RND_SIZE; i++) {
        dest[i] = src[(i + direction + MFDES_RND_SIZE) % 8]; //Adding size to prevent negative numbers
    }
}
...
NfcCommand mfdes_listener_continue_authentication(MfDesDevice* device, const uint8_t data){
    ...
    for(uint8_t i = 0; i < 16; i++) {
        data_to_decrypt[i] = bit_buffer_get_byte(data, i + 6);
    }
    ...
    bit_buffer_append_byte(device->tx_buf, encrypted_rndA);
    bit_buffer_append_byte(device->tx_buf, MFDES_CLASS_RESPONSE);
    bit_buffer_append_byte(device->tx_buf, MFDES_SUCCESS);
...
NfcCommand mfdes_listener_callback(NfcGenericEvent event, void* device){
    ...
    if(Iso14443_4a_event->type == Iso14443_4aListenerEventTypeFieldOff){
        FURI_LOG(TAG, "FieldOff");
        if (context->listener_state == MfDesListenerStateAuthenticated){
            mfdes_on_done(instance);
        }
        else if (context->listener_state != MfDesListenerStateIdle){ //Pokud cokoliv jineho nez idle a authenticated tak je to error
        ...
        FURI_LOG(TAG, "Halted");
        if (context->listener_state == MfDesListenerStateAuthenticated){
            mfdes_on_done(instance); //Pokud by ji uspal po tom co je hotovo tak mi to nevadi a mam done
        }
    }
